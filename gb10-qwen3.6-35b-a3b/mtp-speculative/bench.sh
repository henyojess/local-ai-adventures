#!/bin/bash
# bench.sh — vLLM throughput benchmark with provenance metadata
#
...
#   --image-digest STR   Override auto-detected image digest
#   --max-tokens N       Max output tokens per request (default: 300)
...
die() {
  echo "ERROR: $*" >&2
  echo "" >&2
  echo "Use --image-digest <sha256:...> to bypass auto-detection." >&2
  exit 2
}
...
resolve_image_digest() {
  # Explicit override: caller passed a string; treat it as the image_id
  # and mark the ref as overridden.
  if [[ -n "$IMAGE_DIGEST" ]]; then
    printf '%s\n%s\n' "(override)" "$IMAGE_DIGEST"
    return
  fi

  if ! command -v docker >/dev/null 2>&1; then
    die "docker not found on PATH; cannot resolve container for port ${PORT}."
  fi
...
{ read -r IMAGE_REF; read -r IMAGE_ID; } < <(resolve_image_digest)
...
cat <<META
============================================================
bench.sh v${SCRIPT_VERSION}  label=${LABEL} output=${OUT_FILE}
============================================================
